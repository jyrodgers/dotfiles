...
export HOST_USER_ID=${HOST_USER_ID:-`stat -c %u /root`}
export HOST_GROUP_ID=${HOST_GROUP_ID:-`stat -c %g /root`}

groupadd -g $HOST_GROUP_ID group
useradd -u $HOST_USER_ID -g group root
...
exec /sbin/su-exec root tmux -u -2 new -s ${PROJECT_NAME}
